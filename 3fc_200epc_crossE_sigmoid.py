...
def train_test_split(X, y, test_size=0.2, random_state=13):
...
fc_layer2 = FullyConnectedLayer(sizeIn=32, sizeOut=16)  
sigmoid_layer2 = LogisticSigmoidLayer()  
fc_layer3 = FullyConnectedLayer(sizeIn=16, sizeOut=1) 
...
for epoch in range(epochs):
    ...
    for x, y_true in zip(X_train, y_train):
        ...
        x = sigmoid_layer2.forward(x)
        x = fc_layer3.forward(x)
        ...
        d_loss = fc_layer3.backward(d_loss)
        d_loss = sigmoid_layer2.backward(d_loss)
        ...
        fc_layer3.updateWeights(learning_rate)
    ...
    for x_val, y_val_true in zip(X_test, y_test):
        ...
        x_val = sigmoid_layer2.forward(x_val)
        x_val = fc_layer3.forward(x_val)
        y_val_pred = sigmoid_layer_output.forward(x_val)
...
plt.ylabel('Loss')
plt.legend()
plt.savefig(f'./plots/{no_layer}fc_{epochs}epc_training_validation_loss.png')
