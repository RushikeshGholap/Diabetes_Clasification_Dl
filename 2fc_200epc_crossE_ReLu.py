...
from Activation_Layers.ReLULayer import ReLULayer
import numpy as np
...
for epoch in range(epochs):
    ...
    for x, y_true in zip(X_train, y_train):
        ...
        x = relu_layer1.forward(x)
        x = fc_layer2.forward(x)
        y_pred = relu_layer_output.forward(x)
        ...
        d_loss = relu_layer_output.backward(d_loss,x)
        d_loss = fc_layer2.backward(d_loss)
        d_loss = relu_layer1.backward(d_loss,x)
    ...
    for x_val, y_val_true in zip(X_test, y_test):
        ...
        x_val = relu_layer1.forward(x_val)
        x_val = fc_layer2.forward(x_val)
        y_val_pred = relu_layer1.forward(x_val)
...
plt.title(f'Train and Valid Loss over Epochs; {no_layer} FC Lyr with LR: {learning_rate} & ReLu')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.savefig(f'./plots/{no_layer}fc_{epochs}epc_training_validation_loss_ReLu.png')
