...
fc_layer = FullyConnectedLayer(sizeIn=input_size, sizeOut=1)  # Output layer directly predicts the output
...
for epoch in range(epochs):
    epoch_loss = 0
    for x, y_true in zip(X_train, y_train):  # Make sure to use the training data
        x = x.reshape(1, -1)
        y_true = y_true.reshape(1, -1)
        
        # Forward pass through single FC layer and sigmoid output
        ...
        d_loss = fc_layer.backward(d_loss)
